...
export async function getSessionSummary(userId, sessionId, limit = 10) {
  ...
  try {
    ...
    const summary = `SẢN PHẨM ĐÃ XEM: ${productList.join("; ")} | Tổng: ${lastViewedProducts.length} sản phẩm`;
...
export async function processInput(text, userId, sessionId) {
  ...
  if (index !== null || pronoun !== null) {
    ...
    if (resolution && resolution.success) {
      ...
      if (pronoun) {
        processedText = `${text} [đang đề cập đến: ${resolution.product.name}]`;
      } else if (index) {
        processedText = `${text} [đang đề cập đến: ${resolution.product.name} - vị trí ${index}]`;
      }
